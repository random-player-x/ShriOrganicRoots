...
export function CartPage(){
    const TotalSum = useRecoilValue(TotalSumAtom);
    const productsIds = useRecoilValue(cartAtom); // adding the products cart
    const productArray = useRecoilValue(spiceAtom).filter((p) => {
        return productsIds.includes(p.id); // Explicit return statement
    });
            ...
            <div className="grid grid-cols-12">
                <div
                    ...
                    <div className="flex items-center justify-between pb-8 border-b border-gray-300">
                        ...
                        <h2 className="font-manrope font-bold text-xl leading-8 text-gray-600">{productsIds.length} Items</h2>
                    </div>
                    ...
                    {
                        productArray.map((item)=> <CartComponent key={item.id} id = {item.id} name={item["Product Name"]} price={parseFloat(item["Price per Unit ($)"]).toFixed(2)} category={item.Category}/>)
                    }
                ...
                <div
                    ...
                    <div className="mt-8">
                        <div className="flex items-center justify-between pb-6">
                            <p className="font-normal text-lg leading-8 text-black">{productsIds.length} Items</p>
                            <p className="font-medium text-lg leading-8 text-black">${TotalSum}</p>
                        </div>
                        <form>
                            ...
                            <div className="flex items-center justify-between py-8">
                                <p className="font-medium text-xl leading-8 text-black">{productsIds.length} Items</p>
                                <p className="font-semibold text-xl leading-8 text-indigo-600">${TotalSum}</p>
                            </div>
                            <button
